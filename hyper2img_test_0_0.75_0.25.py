...
with torch.no_grad():
    for i in range(len(testdata)):
        ...
        output_path = img_path.replace('/DehazeFormer/data/D-HAZY/test/hazy/', '/control-net-main-v0.5/output/dhaze/few0_0.75_0.25/')     #f'{output_dir}/{i}.png'   
        if not os.path.exists(output_path):
            samples, intermediates = ddim_sampler.sample(ddim_steps, num_samples,
                                                            shape, cond, verbose=False, eta=eta,
                                                            unconditional_guidance_scale=scale,
                                                            unconditional_conditioning=un_cond,
                                                            hyper_scale=hyper_scale, control_scale=[0.75, 0.25], is_train=False, multi=True) # control_scale=control_scale, multi=True)
